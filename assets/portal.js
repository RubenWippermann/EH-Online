/* erstehilfekurse.online — Portal-Logik: Kursfinder + Rechner (ohne Abhängigkeiten) */
(function () {
  'use strict';
  ...
  function initFinder() {
    ...
    function show(anlassSlug) {
      ...
      result.innerHTML =
        ...
        '<span class="eyebrow">Empfehlung für „' + esc(a.name) + '"</span>' +
        '<h2>' + esc(k.name) + '</h2>' +
        '<p>' + esc(k.kurz) + '</p>' +
        '<dl class="k-meta">' +
        '<div><dt>Dauer</dt><dd>' + esc(k.dauer) + '</dd></div>' +
        '<div><dt>Gültig</dt><dd>' + esc(k.gueltig) + '</dd></div>' +
        '<div><dt>Wer zahlt</dt><dd>' + esc(k.wer_zahlt) + '</dd></div>' +
        '<div><dt>Preis</dt><dd>' + esc(k.preis) + '</dd></div></dl>' +
        '<div class="hero-actions">' +
        '<a class="btn primary" href="' + esc(k.url) + '">Zum Kurs &amp; zu den Terminen →</a>' +
        '<a class="btn ghost" href="/inhouse/#anfrage">Als Kurs vor Ort anfragen</a></div>' +
        '<p class="muted" style="font-size:.86rem;margin-top:12px">Nicht sicher? ' +
        '<a href="tel:+4955277488495">Ruf kurz an</a> — wir sortieren das in zwei Minuten.</p>' +
        '</div>';
    ...
    Array.prototype.forEach.call(tiles, function (t) {
      t.addEventListener('click', function () { show(t.getAttribute('data-anlass')); });
    });
  ...
  function initErsthelfer() {
    ...
    function calc() {
      ...
      if (n <= 20) {
        ...
        var quote = typ === 'sonstige' ? 0.10 : 0.05;
  ...
  function initAuffrischung() {
    ...
    function calc() {
      if (!datum.value) { out.innerHTML = ''; return; }
      var d = new Date(datum.value);
      if (isNaN(d.getTime())) { out.innerHTML = ''; return; }
      ...
      var tage = Math.round((faellig - heute) / 86400000);
      var status, cls;
      if (tage < 0) {
        status = 'Überfällig seit ' + Math.abs(tage) + ' Tagen — die Frist ist abgelaufen. ' +
          'Jetzt ist in der Regel die komplette Erste-Hilfe-Ausbildung erneut nötig, nicht nur die Fortbildung.';
        cls = 'err';
      } else if (tage <= 90) {
      ...
      out.innerHTML =
        '<div class="calc-result"><b class="calc-big">' + fmt(faellig) + '</b>' +
        '<span>spätestens auffrischen (alle ' + jahre + ' Jahre)</span></div>' +
        '<p class="calc-status ' + cls + '">' + status + '</p>' +
        '<a class="btn primary" href="/kurse/erste-hilfe-fortbildung/" style="margin-top:8px">Fortbildungstermine ansehen →</a>';
    }
  ...
  function boot() { initFinder(); initErsthelfer(); initAuffrischung(); }
